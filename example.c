...
/* Advanced READLINE-like completion (see below after main for completion code) */
void completion_rl (const char *text, int start, int end, linenoiseCompletions *lc);
...
int main(int argc, char **argv) {
	...
	if (!strcmp(*argv,"--readline")) {
        ...
        if (!strcmp(*argv,"--multiline")) {
            ...
            printf("Printing keycodes.\n");
            linenoisePrintKeyCodes();
            exit(0);
        } else {
            fprintf(stderr, "Usage: %s [--readline] [--multiline] [--keycodes]\n", prgname);
            exit(1);
        }
    }
...

//
// Much more advanced READLINE-like completion code, with local support function for command/argument/file completion
//
...
//
// Fake commands & arguments to demonstrate simple command and complex directory/file completion
//
...
static char  *file_arg1[]    = { "dirA/", "dirA/fileA1", "dirA/fileA2", "dirA/fileA3",
				 ...
				 "dirB/subdirB2/", "dirB/subdirB2/subfileB2a", "dirB/subdirB2/subfileB2b", "dirB/subdirB2/subfileB2c",
				 "fileC",
				 "fileD",
				 "fileE",
...
//
// File completion support functions
//
...
//
// Filter for file completion (so only basename is shown in completion list
//
char *
...
completion_rl_file (const char *text, char **cmd_arg1, linenoiseCompletions *lc) {
  ...
  for (j = 0; ca1[j]; j++) {
    ...
    if (match) {
      ...
      if (ca1[j][l-1] == '/') {
	linenoiseCompletionAppendChar = 0;
      }
    }
  }
}
...
//
// Readline-like completion function for command/arguments
//
void
completion_rl (const char *text, int start, int end, linenoiseCompletions *lc) {
  char *line = linenoiseCompletionBuffer;
